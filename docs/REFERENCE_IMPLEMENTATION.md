...
文章页在 CSS 视口宽度不小于 `90rem` 时，会把“目录 + 正文”作为一个整体居中：
左侧目录使用 `18rem` 至 `24rem` 的独立空间，正文向右平移；接近显示阈值时正文会
适度收窄以保留两侧安全边距，更宽的屏幕仍使用原页面宽度。这样即使 Windows 显示
缩放或浏览器缩放降低了 CSS 视口宽度，常见桌面窗口仍能显示目录。长章节名在目录中
自动换行并完整显示。目录从正文 `h2` 开始，显示三级层次；滚动时仅高亮当前章节，
点击后平滑滚动并更新 URL 锚点。空间不足及打印时恢复正文居中并隐藏目录，避免覆盖
正文。
